...
def argparser():
    ...
    parser.add_argument('-n','--n-iters',default=int(100),type =int)
    parser.add_argument('-hd','--hidden',default=int(256),type =int)
    parser.add_argument('-pf','--print-freq',default=int(20),type =int)
    parser.add_argument('-b','--batch-size',default=int(256 / 16),type =int)
...
for _ in range(args.n_iters):
    ...
    def benchmark():
        ...
        generated_rnn = np_find_state(generated_rnn.cpu().numpy())
    ...
    if _ % 20 == 0:
        torch.save(RNNMODEL.state_dict() ,
                "{}/{}/RNN_{}_{}_{}.pth".format(\
                    dirName, dataname, dataname, demand_type.split('.')[0], _
                                                    ) 
                )

        print("The model is saved at :: {}/{}/RNN_{}_{}_{}.pth ".format(\
                    dirName, dataname, dataname, demand_type.split('.')[0], _
                                                    ) )

with open("{}/loss/{}_RNN.pkl".format(dirName, dataname), "wb") as f:
    pickle.dump(loss_avg, f)

with open("{}/loss/{}_RNN_bleu.pkl".format(dirName, dataname), "wb") as f:
    pickle.dump(bleu_list, f)

with open("{}/loss/{}_RNN_metor.pkl".format(dirName, dataname), "wb") as f:
    pickle.dump(meteor_list, f)
