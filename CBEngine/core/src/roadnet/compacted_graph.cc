#include "../head/compacted_graph.h"
#include <functional>
#include <iostream>
#include <queue>
#include <set>

void CompactedGraph::InitFromAdjTable(
...
void CompactedGraph::InitFromEdgeList(
    std::vector<std::pair<std::pair<int, int>, double>> edges, int n) {
  std::vector<std::vector<std::pair<int, double>>> adj;
  adj.resize(n);
  for (auto edge : edges) {
...
const std::vector<int> CompactedGraph::FindShortestPath(int from, int to,
                                                  bool is_spfa) const{
...
const std::vector<int> CompactedGraph::FindShortestPathBruteForce(int from, int to,
                                                            bool is_spfa) {
  int m = adj_.size();
  std::vector<double> dis(m, 1e30);
  std::vector<bool> vis(m, false);
...
double CompactedGraph::QueryPathLength(std::vector<int> path) {
  double length = 0;
